yii.gii = (function ($) {
    ...
    var initPreviewDiffLinks = function () {
        $('.preview-code, .diff-code, .modal-refresh, .modal-previous, .modal-next').on('click', function () {
            ...
            ajaxRequest = $.ajax({
                ...
                cache: false,
                url: $link.prop('href'),
                data: $('.default-views form').serializeArray(),
    ...
    var checkAllToggle = function () {
        $('#check-all').prop('checked', !$('.default-views-files table .check input:enabled:not(:checked)').length);
    };

    var initConfirmationCheckboxes = function () {
        var $checkAll = $('#check-all');
        $checkAll.click(function () {
            $('.default-views-files table .check input:enabled').prop('checked', this.checked);
        });
        $('.default-views-files table .check input').click(function () {
            checkAllToggle();
        });
        checkAllToggle();
    };
    ...
    var initToggleActions = function () {
        $('#action-toggle').find(':input').change(function () {
            ...
            var $rows = $('.' + this.value, '.default-views-files table').toggleClass('action-hidden', !this.checked);
            if (this.checked) {
    ...
    return {
        ...
        init: function () {
            ...
            $('#form-fields').find('input,select,textarea').change(function () {
                $('.default-views-results,.default-views-files').hide();
                $('.default-views button[name="generate"]').hide();
            });
